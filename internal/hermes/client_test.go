...
func TestChatCompletion_Success(t *testing.T) {
	...
	client := NewClient(server.URL, "/v1/chat", "test-model", "test-token", "", 30*time.Second)
...
func TestChatCompletion_RequestError(t *testing.T) {
	client := NewClient("http://localhost:99999", "/v1/chat", "test-model", "test-token", "", 1*time.Millisecond)
...
func TestGetContent(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			resp: &agent.ChatCompletionResponse{
				Choices: []agent.Choice{
					{Message: agent.Message{Content: "Hello World"}},
				},
			},
		...
		{
			name:     "empty choices",
			resp:     &agent.ChatCompletionResponse{Choices: []agent.Choice{}},
			expected: "",
		},
		{
			name: "nil content",
			resp: &agent.ChatCompletionResponse{
				Choices: []agent.Choice{
					{Message: agent.Message{Content: nil}},
				},
			},
			expected: "",
		},
